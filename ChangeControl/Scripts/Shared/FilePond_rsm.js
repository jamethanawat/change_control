$(() => {
    ...
    const inputElement = document.querySelector('input[name="filepond_rsm"]');
    var file_length = 0;
    var file_arry = [];
    var file_detail = [];
    var rsm_bad_file = 0;
    var pond_rsm = FilePond.create(inputElement, {
        maxFiles: 20,
        allowMultiple: true,
        required: false,
        allowPaste: false,
    ...
    var count = 0;
    pond_rsm.on('addfile', (error, item) => {
        ...
        // file_arry = [];
        // }
        console.log(file_list_rsm,"upload");
    });

    pond_rsm.on('removefile', (error, item) => {
        if (item.status == 8) {
            rsm_bad_file--;
            if (rsm_bad_file == 0) $(".btn-info").prop("disabled", false);
        }
        if (error) {
            console.log('Oh no');
            return;
        ...
        file_list_rsm = file_list_rsm.filter(e => e.detail.id != item.id);
    });
    ...
    function add_description(update_id) {
       // $(".modal").not("#resubmit_modal").modal("hide")
        ...
        update_i = file_list_rsm.findIndex(item => item.detail.id == update_id);
        swal({
            ...
            content: {
                ...
                attributes: {
                    ...
                    value: file_list_rsm[update_i].description,
                },
            ...
            console.log(file_list_rsm,"file_list_rsm")
            $("#resubmit_modal").modal("show");
            if (result != null && result != "" && result != "clear") {
                console.log(`${update_id}`);
                $(`.filepond--file-status-main#s${update_id}`).html(result).show("slow");
                file_list_rsm[update_i].description = result;
                swal({
                    title: "สำเร็จ",
                ...
                file_list_rsm[update_i].description = null;
            }
        });
    }
